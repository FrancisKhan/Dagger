...
inline std::ostream& operator << (std::ostream& stm, XSKind xsKind)
{
    switch(xsKind)
    {
        ...
        case XSKind::DIFFCOEFF : return stm << "DIFFCOEFF";
...
inline std::string get_name(XSKind xsKind) 
{
  switch (xsKind) 
  {
    ...
    case XSKind::DIFFCOEFF : return std::string("DIFFCOEFF");
...
{
    return XSKind(int(XSKind::DIFFCOEFF) + 1);
}
